...
var Candy = (function(self, $) {
    ...
    self.init = function(service, options) {
        ...
        if(CandyPlus.Colors) {
            CandyPlus.Colors.init();
        }

        if(CandyPlus.Hall) {
            CandyPlus.Hall.init(options.hall);
        }
    };
...
Candy.View = (function(self, $) {
        ...
        _registerToolbarHandlers = function() {
            ...
            var fullscreen = false,
                enableFullScreen = function() {
                    $("#header-wrapper").hide();
                    $("#footer-wrapper").hide();
                    $("#content-wrapper").css({ marginTop: 0, marginBottom: 0 });
                    $("#content-box").css({ width: '100%' });
                    $("#content").css({ top: 0, bottom: 0 });
                },
                disableFullScreen = function() {
                    $("#content").css({ top: 10, bottom: 10 });
                    $("#content-box").css({ width: '960px' });
                    $("#content-wrapper").css({ marginTop: 45, marginBottom: 36 });
                    $("#footer-wrapper").show();
                    $("#header-wrapper").show();
                };
    ...
    self.init = function(container, options) {
        ...
        _current.container.html(Mustache.to_html(Candy.View.Template.Chat.pane, {
            ...
            tooltipFullScreen : $.i18n._('tooltipFullScreen'),
            resourcesPath : this.getOptions().resources
        }, {
            topbar: Candy.View.Template.Chat.topbar,
            tabs: Candy.View.Template.Chat.tabs,
            mainbox: Candy.View.Template.Chat.mainbox,
            sidebars: Candy.View.Template.Chat.sidebars,
            panels: Candy.View.Template.Chat.panels,
            bottombar: Candy.View.Template.Chat.bottombar,
            toolbox: Candy.View.Template.Chat.toolbox,
            editbox: Candy.View.Template.Chat.editbox,
...
Candy.Util = (function(self, $){
    ...
    self.getPosLeftAccordingToWindowBounds = function(elem, pos) {
        ...
        var windowWidth = $("#chat-pane").width(),
        ...
        return { px: pos-$("#chat-pane").offset().left, backgroundPositionAlignment: backgroundPositionAlignment };
    };
    ...
    self.getPosTopAccordingToWindowBounds = function(elem, pos) {
        ...
        var windowHeight = $("#chat-pane").height(),
        ...
        return { px: pos-$("#chat-pane").offset().top, backgroundPositionAlignment: backgroundPositionAlignment };
    };
            ...
            if(args.type === 'connection') {
                switch(args.status) {
                    ...
                    case Strophe.Status.DISCONNECTED:
                        ...
                        Candy.View.Pane.Chat.Modal.showOfflinePane($.i18n._('statusDisconnected'), presetJid);
...
Candy.View.Pane = (function(self, $) {
    ...
    self.Chat = {
        ...
        tabClick: function(e) {
            ...
            if(self.Chat.rooms[currentRoomJid]) {
        ...
        Modal: {
             ...
             */
            showOfflinePane: function(message, presetJid) {
                self.Chat.Modal.show((message ? message : '') + Mustache.to_html(Candy.View.Template.Offline.pane, {
                    _offlineHint: $.i18n._('offlineHint'),
                }));
            },
        ...
        Context: {
            ...
            showEmoticonsMenu: function(elem) {
                ...
                content.find('img').click(function() {
                    var input = Candy.View.Pane.Room.getPane(Candy.View.getCurrent().roomJid, '.message-form').find('.field'),
                        value = input.val(),
    ...
    self.Room = {
        ...
        init: function(roomJid, roomName, roomType) {
            ...
            $('#roster-panes').append(Mustache.to_html(Candy.View.Template.Roster.pane, {
                roomId: roomId,
            ...
            $('#message-forms').append(Mustache.to_html(Candy.View.Template.Room.form, {
                roomId: roomId,
        ...
        show: function(roomJid) {
            ...
            $('.roster-pane').each(function() {
                var elem = $(this);
                if(elem.attr('id') === ('roster-pane-' + roomId)) {
                    elem.show();
                } else {
                    elem.hide();
                }
            ...
            $('.message-form-wrapper').each(function() {
                var elem = $(this);
                if(elem.attr('id') === ('message-form-' + roomId)) {
                    elem.show();
                } else {
                    elem.hide();
                }
        ...
        close: function(roomJid) {
            ...
            */
            self.Room.getPane(roomJid, '.message-form').parent().remove();
            self.Room.getPane(roomJid, '.roster-pane').remove();
         ...
         * Use cases:
         *   getPane(roomJid)
         *   getPane(roomJid, '.message-pane-wrapper')
         *   getPane(roomJid, '.message-pane')
         *   getPane(roomJid, '.message-form')
         *   getPane(roomJid, '.roster-pane')
         */
        getPane: function(roomJid, subPane) {
            if (self.Chat.rooms[roomJid]) {
                if(subPane) {
                    if(subPane == '.message-form') {
                        var roomId = Candy.Util.jidToId(roomJid);
                        return $('#message-form-' + roomId + ' .message-form');
                    }
                    else if(subPane == '.roster-pane') {
                        var roomId = Candy.Util.jidToId(roomJid);
                        return $('#roster-pane-' + roomId);
                    }

                    // subPane == '.message-pane-wrapper' | '.message-pane'
    ...
    self.PrivateRoom = {
        ...
        setStatus: function(roomJid, status) {
            ...
            if(status === 'join') {
                ...
                messageForm.find('.field').removeAttr('disabled');
                ...
                messageForm.find('.field').attr('disabled', true);
...
Candy.View.Template = (function(self){
    ...
    self.Chat = {
        pane: '<div id="chat-pane">{{> topbar}} {{> mainbox}} {{> bottombar}}</div>{{> modal}}',
        topbar: '<div id="tabs-box"><div>{{> tabs}}</div></div>',
        tabs: '<ul id="chat-tabs"></ul>',
        tab: '<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}"><a href="#" class="close">\u00D7</a><a href="#" class="label">{{#privateUserChat}}@{{/privateUserChat}}{{name}}</a><a href="#" class="transition"></a><small class="unread"></small></li>',
        mainbox: '<div id="main-box">{{> sidebars}}{{> panels}}</div>',
        sidebars: '<div id="sidebars"><div id="roster-panes"></div></div>',
        panels: '<div id="panels"><div id="chat-rooms"></div></div>',
        bottombar: '<div id="toolbar">{{> toolbox}}{{> editbox}}</div>',
        toolbox: '<div id="tool-box">{{> toolbar}}</div>',
        editbox: '<div id="edit-box"><div id="message-forms"></div></div>',
    ...
    self.Room = {
        pane: '<div class="room-pane roomtype-{{roomType}}" id="chat-room-{{roomId}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}">{{> messages}}</div>',
        subject: '<dt>{{time}}</dt><dd class="subject"><span class="label">{{roomName}}</span>{{_roomSubject}} {{subject}}</dd>',
        form: '<div class="message-form-wrapper" id="message-form-{{roomId}}"><form method="post" class="message-form"><input type="submit" class="submit" name="submit" value="{{_messageSubmit}}" /><div class="chat-edit-box"><input name="message" class="field" type="text" autocomplete="off" maxlength="1000" /></div></form></div>'
    };

    self.Roster = {
        pane: '<div class="roster-pane" id="roster-pane-{{roomId}}"></div>',
    ...
    self.Offline = {
        pane: '<div id="offline-pane">{{_offlineHint}}</div>'
    };
...
Candy.View.Translation = {
    'en' : {
        ...
        'tooltipFullScreen'       : 'Toggle full screen',
        ...
        'offlineHint': 'The session was replaced by another connection, or a network error occured. You can refresh the page or try again later.'
    },
    ...
    'cn': {
        ...
        'userLeftRoom': '%s 离开房间',
        'userHasBeenKickedFromRoom': '%s 被请出此房间',
        ...
        'tooltipStatusmessage': '加入/离开房间提示',
        'tooltipAdministration': '房间管理',
        'tooltipUsercount': '房间参与者',
        'tooltipFullScreen' : '切换全屏',

        'enterRoomPassword': '加入房间 "%s" 需要密码.',
        'enterRoomPasswordSubmit': '加入房间',
        'passwordEnteredInvalid': '加入房间 "%s" 的密码不正确',
        ...
        'offlineHint': '可能因为此帐号在别处登录了，或者发生了网络错误。你可以刷新页面或者稍后重试。'
    },
...
// CandyPlus: colors
var CandyPlus = (function(self) { return self; }(CandyPlus || {}));

CandyPlus.Colors = (function(self, Candy, $) {
    ...
    self.init = function(numColors) {
        ...
        if(Candy.Util.cookieExists('candyplus-colors-current')) {
            var color = parseInt(Candy.Util.getCookie('candyplus-colors-current'), 10);
        ...
        $('#colors-control').click(function(event) {
            CandyPlus.Colors.showPicker(this);
        });
    };
    ...
    self.showPicker = function(elem) {
        ...
        content.find('span').click(function() {
            ...
            Candy.Util.setCookie('candyplus-colors-current', _currentColor, 365);
    ...
    self.applyTranslations = function() {
        ...
        Candy.View.Translation.cn.candyshopColorsMessagecolor = '文字颜色';
        Candy.View.Translation.ja.candyshopColorsMessagecolor = 'Message color';
    };

    return self;
}(CandyPlus.Colors || {}, Candy, jQuery));

CandyPlus.Hall = (function(self, Candy, Strophe, $) {
    self.roomJid  = 'rooms.index';
    self.roomName = 'Index';
    self.roomType = 'fake';
    self.roomId   = Candy.Util.jidToId(self.roomJid);
    ...
    self.initUI = function() {
        ...
        var tab_html = Mustache.to_html(self.Template.tab, {
                roomJid: roomJid,
                roomId: roomId,
                name: roomName,
                privateUserChat: function() { return roomType === 'chat'; },
            ...
            tab = $(tab_html).appendTo('#chat-tabs');
        Candy.View.Pane.Chat.fitTabs();
        tab.click(self.showRoomList);

        var pane_html = Mustache.to_html(self.Template.pane, {
                roomJid: roomJid,
                roomId: roomId,
                roomType: roomType,
                _candyplusLoadingRoomList: $.i18n._('candyplusLoadingRoomList')
            }),
            pane = $(pane_html).appendTo('#chat-rooms');

        var roster_html = Mustache.to_html(self.Template.roster, {
                roomId: roomId
            }),
            roster = $(roster_html).appendTo('#roster-panes');

        var form_html = Mustache.to_html(self.Template.form, {
                roomId: roomId,
                _candyplusRoomPanelChooseRoom: $.i18n._('candyplusRoomPanelChooseRoom')
            }),
            form = $(form_html).appendTo('#message-forms');

        form.find("form").submit(function(event) {
            event.preventDefault();

            var room_name = $.trim(form.find("#new_room_name").val());
            if(room_name === '') {
                return false;
            }
            else if(room_name.length > 15) {
                alert($.i18n._('candyplusRoomNameTooLong'));
                return false;
            }
            ...
            if(room_name.search(/[:@&"'\/\\<>]/) >= 0) {
                alert($.i18n._('candyplusRoomNameIllegal'));
                return false;
            }

            var roomJid = room_name + '@' + _options.mucDomain;
            Candy.Core.Action.Jabber.Room.Join(roomJid);
        });
    };
    ...
    self.init = function(options) {
        ...
        if (_options.showIfAllTabClosed) {
            Candy.View.Pane.Chat.allTabsClosed = function () {
                CandyPlus.Hall.showRoomList();
                return;
            };
        } //if

        // Overwrite candy View.Pane.Room.show function
        // Take special care when roomJid == CandyPlus.Hall.roomJid
        var old_showRoom = Candy.View.Pane.Room.show,
            new_showRoom = function(roomJid) {
                if(CandyPlus.Hall.roomJid == roomJid) {
                    window.CandyPlus.Hall.showRoomList();
                    return;
                } else {
                    old_showRoom(roomJid);
                }
            };
        Candy.View.Pane.Room.show = new_showRoom;

        Candy.Core.Event.addObserver(Candy.Core.Event.KEYS.CHAT, {update: function(obj, data) {
            if (data.type == 'connection') {
                // Strophe.Status.CONNECTED = 5
                // Strophe.Status.ATTACHED = 8
                if (data.status == 8 || data.status == 5) {
                    /* only show room window if not already in a room, timeout is to let some time for auto join to execute */
                    setTimeout(CandyPlus.Hall.showRoomPanelIfAllClosed, 500);
                } //if
            } //if
            return true;
        }});
    ...
    self.show = function() {
        $('.room-pane').each(function() {
            var elem = $(this);
            if(elem.attr('id') === ('chat-room-' + self.roomId)) {
                elem.show();
                Candy.View.getCurrent().roomJid = self.roomJid;
                Candy.View.Pane.Chat.updateToolbar(null);  // hide tool bar
                Candy.View.Pane.Chat.setActiveTab(self.roomJid);
                Candy.View.Event.Room.onShow({'roomJid': self.roomJid, 'element' : elem});
            } else {
                elem.hide();
                Candy.View.Event.Room.onHide({'roomJid': self.roomJid, 'element' : elem});
            }
        });
        
        $('.roster-pane').each(function() {
            var elem = $(this);
            if(elem.attr('id') === ('roster-pane-' + self.roomId)) {
                elem.show();
            } else {
                elem.hide();
            }
        ...
        $('.message-form-wrapper').each(function() {
            var elem = $(this);
            if(elem.attr('id') === ('message-form-' + self.roomId)) {
                elem.show();
            } else {
                elem.hide();
            }
    ...
    self.updateRoomList = function (iq) {  // $iq callback
        var newRoomList = [];
    ...
    self.showRoomList = function() {
            ...
            if ($('#chat-modal').is(':visible')) {
                setTimeout(CandyPlus.Hall.showRoomList, 100);
            }
            else {
                ...
                if (_options.autoDetectRooms && timeDiff > _lastRoomUpdate ) {
                    ...
                    var iq = $iq({type: 'get', from: Candy.Core.getUser().getJid(), to: _options.mucDomain  , id: 'findRooms1'})
                        .c('query', {xmlns: Strophe.NS.DISCO_ITEMS});   // construct iq
                    
                    Candy.Core.getConnection().sendIQ(iq, self.updateRoomList);  // send iq
                }
                else {
                    var html = Mustache.to_html(CandyPlus.Hall.Template.rooms, {
                        title: $.i18n._('candyplusRoomPanelListRoom'),
                        roomList: _options.roomList
                    });

                    $('#chat-room-' + self.roomId).html(html);
    ...
    self.applyTranslations = function() {
            Candy.View.Translation.en.candyplusRoomPanelListRoom = 'List Rooms';
            Candy.View.Translation.cn.candyplusRoomPanelListRoom = '房间列表';
            Candy.View.Translation.ja.candyplusRoomPanelListRoom = 'List Rooms';

            Candy.View.Translation.en.candyplusLoadingRoomList = 'Loading room list...';
            Candy.View.Translation.cn.candyplusLoadingRoomList = '正在获取房间列表...';
            Candy.View.Translation.ja.candyplusLoadingRoomList = 'Loading room list...';

            Candy.View.Translation.en.candyplusRoomPanelChooseRoom = 'Choose Room To Join, or Create New Room';
            Candy.View.Translation.cn.candyplusRoomPanelChooseRoom = '选择要加入的房间，或者新建房间';
            Candy.View.Translation.ja.candyplusRoomPanelChooseRoom = 'Choose Room To Join, or Create New Room';

            Candy.View.Translation.en.candyplusRoomNameTooLong = 'Room name is too long!';
            Candy.View.Translation.cn.candyplusRoomNameTooLong = '房间名字太长了!';
            Candy.View.Translation.ja.candyplusRoomNameTooLong = 'Room name is too long!';

            Candy.View.Translation.en.candyplusRoomNameIllegal = 'Room name Error! Don\'t use blank space or " \' : @ & < > \\ /';
            Candy.View.Translation.cn.candyplusRoomNameIllegal = '房间名字错误! 不能使用 空格 以及这些符号 " \' : @ & < > \\ /';
            Candy.View.Translation.ja.candyplusRoomNameIllegal = 'Room name Error! Don\'t use blank space or " \' : @ & < > \\ /';
    };

    self.Template = {
        tab: '<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}"><a id="index-room-name" href="#" class="label">{{#privateUserChat}}@{{/privateUserChat}}{{name}}</a></li>',
        pane: '<div class="room-pane roomtype-{{roomType}}" id="chat-room-{{roomId}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}"><div class="room-list-prelude">{{_candyplusLoadingRoomList}}</div></div>',
        roster: '<div class="roster-pane" id="roster-pane-{{roomId}}"></div>',
        form: '<div class="message-form-wrapper hall-form" id="message-form-{{roomId}}"><span id="choose-room-hint">{{_candyplusRoomPanelChooseRoom}}</span><form id="create-room"><input type="text" name="new_room_name" id="new_room_name" /><input id="new_room_submit" type="submit" name="submit" value="OK" /></form></div>',
        rooms: '<div class="room-list"><h2>{{title}}</h2><ul>{{#roomList}}<li class="room-item"><a href="#{{jid}}">{{name}}</a></li>{{/roomList}}</ul></div>'
    };

    return self;
}(CandyPlus.Hall || {}, Candy, Strophe, jQuery));

/*
$(document).ready(function() {
    Candy.init('/http-bind/', {
        core: { debug: true },
        autojoin: ['xmpp@conference.localhost'],
        view: { resources: '/candy/res/', language: 'cn' }
    });

    CandyPlus.Colors.init();

    CandyPlus.Hall.init({ mucDomain: 'conference.localhost' });
    
    //Candy.Core.connect('foo@localhost', '123456');
    //self.attach = function(jid, sid, rid) {
    //Candy.Core.connect();

    Candy.Core.attach('foo@localhost/candy', "43839e10742d01066273e28af47d14325ce723d4", 38058);
});
*/
